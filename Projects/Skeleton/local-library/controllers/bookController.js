...
exports.book_delete_get = function(req, res, next) {
  Book.findById(req.params.id)
      ...
      .then((book) => {
          ...
          // Check if there are associated BookInstances
          BookInstance.find({ book: req.params.id })
              .exec()
              .then((bookinstances) => {
                  if (bookinstances.length > 0) {
                      // Render the page with associated BookInstances
                      res.render('book_delete', { book: book, bookinstances: bookinstances });
                  } else {
                      // Render the page with option to delete the book
              ...
              .then((bookinstances) => {
                  if (bookinstances.length > 0) {
                      ...
                      Book.findByIdAndRemove(req.body.bookid)
                          .exec()
                          .then(() => {
                              // Successful deletion, redirect to book list
                              res.redirect('/catalog/books');
                          })
                          .catch((err) => next(err));
                  }
